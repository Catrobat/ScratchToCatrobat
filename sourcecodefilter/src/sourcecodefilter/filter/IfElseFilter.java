...
public class IfElseFilter extends ASTVisitor {

	final private ConverterRelevantCatroidSource catroidSource;
	final private Set<String> ifElseBlocksToBeRemoved;

	public IfElseFilter(ConverterRelevantCatroidSource catroidSource, Set<String> ifElseBlocksToBeRemoved) {
		this.catroidSource = catroidSource;
		this.ifElseBlocksToBeRemoved = ifElseBlocksToBeRemoved;
	}

    @SuppressWarnings("unchecked")
	public void removeUnallowedIfElseBlocks() {
	...
	public boolean visit(final IfStatement node) {
		...
		for (String expectedExpression : ifElseBlocksToBeRemoved) {
			if (expression.equals(expectedExpression)) {
				node.delete();
			}
		}
		return false;
