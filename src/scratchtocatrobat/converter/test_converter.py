...
def create_catrobat_sprite_stub(name=None):
    sprite = SpriteFactory().newInstance(SpriteFactory.SPRITE_BASE, "Dummy" if name is None else name)
...
class TestConvertBlocks(common_testing.BaseTestCase):
    ...
    def get_sprite_with_soundinfo(self, soundinfo_name):
        ...
        dummy_sprite = SpriteFactory().newInstance(SpriteFactory.SPRITE_BASE, "TestDummy")
    ...
    def test_convert_create_clone_of_block_previous(self):
        sprite_object = SpriteFactory().newInstance(SpriteFactory.SPRITE_BASE, "Previous")
    ...
    def get_color_formulas(set_color_brick):
        r = set_color_brick.getFormulaWithBrickField(catbricks.Brick.BrickField.PEN_COLOR_RED).getRoot()
        g = set_color_brick.getFormulaWithBrickField(catbricks.Brick.BrickField.PEN_COLOR_GREEN).getRoot()
        b = set_color_brick.getFormulaWithBrickField(catbricks.Brick.BrickField.PEN_COLOR_BLUE).getRoot()
        return (r, g, b)
    ...
    def assert_hsv_to_rgb_algorithm(sprite, bricks):
        ...
        assert TestConvertBlocks.is_same_formula(test_fe, bricks[0].getFormulas()[0].getRoot())
        ...
        assert TestConvertBlocks.is_same_formula(test_fe, bricks[1].getFormulas()[0].getRoot())
        ...
        assert TestConvertBlocks.is_same_formula(test_fe, bricks[2].getFormulas()[0].getRoot())
        ...
        assert TestConvertBlocks.is_same_formula(test_fe, bricks[3].getFormulas()[0].getRoot())
        ...
        assert TestConvertBlocks.is_same_formula(test_fe, bricks[4].getFormulas()[0].getRoot())
        ...
        def test_inner_bricks(if_brick, r, g, b):
            for (i, (color, name)) in enumerate([(r, 'r'), (g, 'g'), (b, 'b')]):
                ...
                assert TestConvertBlocks.is_same_formula(brick.getFormulas()[0].getRoot(), fe_test)
        ...
        assert TestConvertBlocks.is_same_formula(test_fe, bricks[5].getFormulas()[0].getRoot())
        ...
        for i in range(1, 6):
            ...
            assert TestConvertBlocks.is_same_formula(fe_test, bricks[5 + i].getFormulas()[0].getRoot())
    ...
    def test_can_convert_set_pen_color_block_with_conversion_algorithm(self):
        ...
        def assert_set_hsv_bricks(bricks, h, s, v):
            ...
            for (brick, variable_name, variable_formula) in zip(bricks, ['h', 's', 'v'], [h, s, v]):
                ...
                assert self.is_same_formula(test_fe, brick.getFormulas()[0].getRoot())
    ...
    def test_can_convert_set_pen_param_to(self):
        ...
        def test_bricks(bricks, affected_variable_name, test_fe):
            ...
            assert self.is_same_formula(test_fe, bricks[0].getFormulas()[0].getRoot())
    ...
    def test_can_convert_set_pen_size_block_with_number(self):
        ...
        assert self.is_same_formula(test_fe, catr_brick.getFormulas()[0].getRoot())
    ...
    def can_convert_pen_size_blocks(self, test_val1, test_val2):
        ...
        assert self.is_same_formula(fe_test, change_var_brick.getFormulas()[0].getRoot())
        ...
        assert self.is_same_formula(fe_test, set_var_brick.getFormulas()[0].getRoot())
    ...
    def test_can_convert_set_variable_with_list(self):
        ...
        assert catr_bricks[0].getFormulas()[0].getRoot().getElementType() == catElementType.USER_LIST
        assert catr_bricks[0].getFormulas()[0].getRoot().getValue() == self._name_of_test_list
    ...
    def test_can_convert_call_block_user_script_already_defined_simple(self):
        function_header = "number1 %n number 1"
        ...
        assert isinstance(catr_bricks[0], catbricks.UserDefinedBrick)
        user_brick = catr_bricks[0]

        definition_brick_elements = user_brick.getUserDefinedBrickDataList()
        assert len(definition_brick_elements) == 3
        assert isinstance(definition_brick_elements[0], userbricks.UserDefinedBrickLabel)

        assert definition_brick_elements[0].type == userbricks.UserDefinedBrickData.UserDefinedBrickDataType.LABEL
        assert definition_brick_elements[0].label == "number1"
        assert isinstance(definition_brick_elements[1], userbricks.UserDefinedBrickInput)
        assert definition_brick_elements[1].type == userbricks.UserDefinedBrickData.UserDefinedBrickDataType.INPUT
        assert definition_brick_elements[1].name.input == "param1"
        assert isinstance(definition_brick_elements[2], userbricks.UserDefinedBrickLabel)
        assert definition_brick_elements[2].type == userbricks.UserDefinedBrickData.UserDefinedBrickDataType.LABEL
        assert definition_brick_elements[2].label == "number 1"

        # TODO sollten im SCF brickfield INPUT_FORMULA_FIELD anlegen, sonst kann man das formula field nicht testen
        # brick_field = userbricks.InputFormulaField(expected_param_label)
        # #user_brick.addAllowedBrickField(brick_field)
        # formula_tree = user_brick.getFormulaWithBrickField(brick_field).formulaTree # @UndefinedVariable
        # assert catformula.FormulaElement.ElementType.NUMBER == formula_tree.type
        # print ("ok",formula_tree.value)
        ...
        assert isinstance(user_script_definition_brick, catbricks.UserDefinedBrick)
    ...
    def test_can_convert_call_block_user_script_already_defined_complex(self):
        function_header = "number 1 %n string1 %s boolean1 %b"
        ...
        assert isinstance(definition_brick_elements[0], userbricks.UserDefinedBrickLabel)
        assert definition_brick_elements[0].type == userbricks.UserDefinedBrickData.UserDefinedBrickDataType.LABEL
        assert definition_brick_elements[0].label == "number 1"
        assert isinstance(definition_brick_elements[1], userbricks.UserDefinedBrickInput)
        assert definition_brick_elements[1].type == userbricks.UserDefinedBrickData.UserDefinedBrickDataType.INPUT
        assert definition_brick_elements[1].name.input == "param1"
        assert isinstance(definition_brick_elements[2], userbricks.UserDefinedBrickLabel)
        assert definition_brick_elements[2].type == userbricks.UserDefinedBrickData.UserDefinedBrickDataType.LABEL
        assert definition_brick_elements[2].label == "string1"
        assert isinstance(definition_brick_elements[3], userbricks.UserDefinedBrickInput)
        assert definition_brick_elements[3].type == userbricks.UserDefinedBrickData.UserDefinedBrickDataType.INPUT
        assert definition_brick_elements[3].name.input == "param2"
        assert isinstance(definition_brick_elements[4], userbricks.UserDefinedBrickLabel)
        assert definition_brick_elements[4].type == userbricks.UserDefinedBrickData.UserDefinedBrickDataType.LABEL
        assert definition_brick_elements[4].label == "boolean1"
        assert isinstance(definition_brick_elements[5], userbricks.UserDefinedBrickInput)
        assert definition_brick_elements[5].type == userbricks.UserDefinedBrickData.UserDefinedBrickDataType.INPUT
        assert definition_brick_elements[5].name.input == "param3"
    ...
    def test_can_convert_call_block_user_script_not_yet_defined_simple(self):
        ...
        expected_user_script_definition_brick = catbricks.UserDefinedBrick()
        user_script_definition_brick_elements_list = expected_user_script_definition_brick.getUserDefinedBrickDataList()

        first_user_script_definition_brick_element = userbricks.UserDefinedBrickLabel("number1")
        user_script_definition_brick_elements_list.add(first_user_script_definition_brick_element)

        second_user_script_definition_brick_element = userbricks.UserDefinedBrickInput(expected_param_label)
        user_script_definition_brick_elements_list.add(second_user_script_definition_brick_element)

        third_user_script_definition_brick_element = userbricks.UserDefinedBrickLabel("number1")
    ...
    def test_can_convert_go_to_sprite_block_with_random_position(self):
        ...
        assert(x_formula.getRoot().value == str(catformula.Functions.RAND))
        assert(y_formula.getRoot().value == str(catformula.Functions.RAND))
    ...
    def test_can_convert_point_towards_block_existing_sprite(self):
        ...
        scratch_block = ["pointTowards:", test_sprite_name]
        sprite = SpriteFactory().newInstance(SpriteFactory.SPRITE_BASE, test_sprite_name)
    ...
    def test_can_convert_dragmode_block(self):
        ...
        assert setBrick.getFormulas()[0].getRoot().getValue() == "TRUE"
        assert setBrick.getFormulas()[0].getRoot().getElementType() == catElementType.FUNCTION
        ...
        assert(setBrick.getUserVariable().getName() == "draggable")
        assert(len(setBrick.getFormulas()) == 1)
        assert(setBrick.getFormulas()[0].getRoot().getValue() == "FALSE")
        assert(setBrick.getFormulas()[0].getRoot().getElementType() == catElementType.FUNCTION)
        assert isinstance(setBrick, catbricks.SetVariableBrick)
        assert len(sprite.getScriptList()) == num_scripts+1
...
class TestConvertedProjectAppendedKeySpriteScripts(common_testing.ProjectTestCase):
    ...
    def setup_key_pressed(self, project_name, key_row_visibility_user_variable_name, key_name):
        ...
        elif project_name == "key_pressed_visibility":
            assert len(default_scene.spriteList) == 4
            keys = default_scene.spriteList[3]
        ...
        index_vis, index_clone_script_brick = 0, 0
        brick_steps = 2
        for _ in range(len(key_row_visibility_user_variable_name)):
            self.setup_check_if_branch_condition(clone_script_bricks[index_clone_script_brick])
            self.setup_clone_script_visibility(key_row_visibility_user_variable_name[index_vis],
                                               clone_script_bricks[index_clone_script_brick],
                                               (clone_script_bricks[index_clone_script_brick].ifBranchBricks.get(3).loopBricks.get(0)))
            self.setup_check_if_branch_condition(clone_script_bricks[index_clone_script_brick + 1])
            self.setup_clone_script_key(clone_script_bricks[index_clone_script_brick + 1],
                                        (clone_script_bricks[index_clone_script_brick+1].ifBranchBricks.get(3)).loopBricks.get(0), key_name[index_vis],
                                        key_row_visibility_user_variable_name[index_vis])
            index_vis += 1
            index_clone_script_brick += brick_steps

    def setup_check_if_branch_condition(self, clone_script_brick):
        first_if_branch = clone_script_brick
    ...
    def setup_clone_script_visibility(self, key_row_visibility_user_variable_name, first_if_branch, wait_until_touches_brick):
    ...
    #--------------------------------clone_script_key---------------------------------------------------------------
    def setup_clone_script_key(self, second_if_branch, wait_until_touches_brick, key_name, key_row_visibility_user_variable_name):
    ...
    def test_key_pressed_visibility(self):
        project_name = 'key_pressed_visibility'
        key_row_visibility_user_variable_name = ["key_row1_visibility", "key_row2_visibility", "key_row3_visibility", "key_row4_visibility", "key_row5_visibility"]
        key_name = ["key_up arrow", "key_x", "key_s", "key_w", "key_1"]
        self.setup_key_pressed(project_name, key_row_visibility_user_variable_name, key_name)
