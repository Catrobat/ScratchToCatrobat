...
class Object(common.DictAccessWrapper):
    ...
    def preprocess_object(self, all_sprite_names):
        ...
        '''
        def replace_change_pen_size_blocks(block_list):
            new_block_list = []
            for block in block_list:
                if isinstance(block, list):
                    if block[0] == "changePenSizeBy:":
                        new_block_list += [["readVariable", S2CC_PEN_SIZE_VARIABLE_NAME]]
                    else:
                        new_block_list += [replace_change_pen_size_blocks(block)]
                else:
                    new_block_list += [block]
            return new_block_list
        '''
